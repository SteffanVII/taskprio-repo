...
import { TGetDeactivatedProjectsRequestParams, TGetDeactivatedprojectsResponseData, TGetProjectListWithUserAssignedTasksRequestPathParams, TGetProjectListWithUserAssignedTasksResponseData, TGetProjectMembersResponseData, TProject, TProjectMember, TProjectPrimitive } from "@repo/taskprio-types"
...
//-- Get user projects by workspace

export const useGetUserProjectsByWorkspace = (_workspace_id?: string) => {

  const selectedWorkspace = useWorkspaceStore_selectedWorkspace()
  const workspaceId = _workspace_id || selectedWorkspace?.workspace_id;

  return useQuery<TProject[], Error>({
    queryKey: [...QueryKeys.GET_USER_PROJECTS_BY_WORKSPACE.split, workspaceId],
    queryFn: async () => {
      const response = await axiosInstance.get<TProject[]>(
        `/private/project/s/${workspaceId}`
      )
      return response.data
    },
    enabled: !!workspaceId
...
export const useUpdateUserProjectsByWorkspaceData = ( _workspace_id? : string ) => {
  ...
  return ( projects? : TProject[] ) => {
    queryClient.setQueryData(
      [...QueryKeys.GET_USER_PROJECTS_BY_WORKSPACE.split, workspaceId],
      () => {
        return projects
      }
    )
...
export const useResetGetUserProjectsByWorkspace = (_workspaceId?: string) => {
  const selectedWorkspace = useWorkspaceStore_selectedWorkspace()
  const worksapceId = _workspaceId || selectedWorkspace?.workspace_id
  return async () => {
    if (!worksapceId) return
    await useQueryClient().resetQueries({
      queryKey : [...QueryKeys.GET_USER_PROJECTS_BY_WORKSPACE.split, worksapceId]
    })
  }
}
