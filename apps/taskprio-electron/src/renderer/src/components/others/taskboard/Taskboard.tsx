...
import { useTaskboardDragStore, useTaskboardDragStore_taskboardTaskDrag } from "@/stores/taskboardDrag";
...
export const Taskboard = () => {
  ...
  const updateTaskboardTaskDrag = useTaskboardDragStore(state => state.updateTaskboardTaskDrag)
  const updateTaskboardSectionDrag = useTaskboardDragStore(state => state.updateTaskboardSectionDrag)
  ...
  const {
    ...
    options: {
      enabled: !!selectedTaskboard && !!taskboard_id
    }
  })
  ...
  const showSkeleton = useMemo(() => {
    return (taskboardsIsLoading || taskboardSectionsIsLoading || !taskboard_id)
  }, [
    taskboardsIsLoading,
  ...
  const onDndContextDragStart = (e: DragStartEvent) => {
    if (e.active.data.current && e.active.data.current.type === ETaskboardDragDataType.TASK) {
      updateTaskboardTaskDrag(e.active.data.current.data as TTaskForCardView)
    }
    if (e.active.data.current && e.active.data.current.type === ETaskboardDragDataType.SECTION) {
      updateTaskboardSectionDrag(e.active.data.current.data as TTaskSection)
    }
  }

  const onDndContextDragEnd = (e: DragEndEvent) => {

    updateTaskboardTaskDrag(null)
    updateTaskboardSectionDrag(null)
  ...
  return (
    ...
    >
      ...
      >
        ...
        {
          (!showSkeleton && taskboard_id && taskboardSections && taskboardSections.length > 0) &&
          <DndContext
            sensors={sensors}
