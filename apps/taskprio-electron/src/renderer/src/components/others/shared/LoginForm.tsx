...
const LoginForm: React.FC<TLoginFormProps> = ({
  ...
  const { openExternalBrowser } = useContext(StateManager_ElectronContext);
  ...
  const openGoogleLoginPage = async () => {
    const PKCE = await window.electronAPI.getPKCE();
    const authUrl = new URL("https://accounts.google.com/o/oauth2/v2/auth")
    authUrl.searchParams.append("client_id", import.meta.env.VITE_GOOGLE_AUTH_CLIENT_ID)
    authUrl.searchParams.append("redirect_uri", "https://taskprio-repo.onrender.com/redirect/google_login")
